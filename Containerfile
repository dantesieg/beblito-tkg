...
RUN curl -Lo /usr/bin/copr https://raw.githubusercontent.com/ublue-os/COPR-command/main/copr && \
    ...
    curl -Lo /etc/yum.repos.d/whitehara-kernel-tkg-preempt-fedora-40.repo https://copr.fedorainfracloud.org/coprs/whitehara/kernel-tkg-preempt/repo/fedora-40/whitehara-kernel-tkg-preempt-fedora-40.repo && \
...
RUN rpm-ostree override replace --experimental --from repo='copr:copr.fedorainfracloud.org:whitehara:kernel-tkg-preempt' kernel kernel-core kernel-modules kernel-modules-core kernel-modules-extra && ostree container commit
...
RUN rpm-ostree install fontconfig-font-replacements fontconfig-enhanced-defaults distrobox jetbrains-mono-fonts && ostree container commit
